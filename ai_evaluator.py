...
def _get_votes_with_split(model, question, batch, leniency, is_auto):
    try:
        votes = get_model_vote(model, question, batch, leniency, allow_fallback_no=False)
        ...
        mid = len(batch) // 2
        left = _get_votes_with_split(model, question, batch[:mid], leniency, is_auto)
        right = _get_votes_with_split(model, question, batch[mid:], leniency, is_auto)
        return left + right
...
def get_model_vote(model, question, answers, leniency, retries=3, allow_fallback_no=True):
    """
...
def evaluate_answers_batch(question, answers, expected=None):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for batch_idx, batch in enumerate(batches, 1):
            ...
            for model in judges:
                future = executor.submit(_get_votes_with_split, model, question, batch, LENIENCY, is_auto)
